...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {//NSLog(@"%@",[NSBundle mainBundle]);
    self.window = [[UIWindow alloc]initWithFrame:[UIScreen mainScreen].bounds];
    self.window.rootViewController = [[XMGFriendTrendViewController alloc]initWithNibName:@"XMGFriendTrendViewController" bundle:nil];
...
-(XMGTabBarController *)manager
{
    ...
    //    [v addSubview:tabBar];
        //[self.window addSubview:tabBar.viewControllers[0]];
        //[self.window addSubview:tabBar];
        _manager=tabBar;
    }
    return _manager;
}
-(XMGNavigationController *)tableview
{
    if (!_tableview) {
        GMLHighFrequencyListViewcontroller *high=[[GMLHighFrequencyListViewcontroller alloc]init];
        XMGNavigationController *tbv=[[XMGNavigationController alloc]initWithRootViewController:high];
        _tableview = tbv;
    }
    return _tableview;
}
...
- (NSURL *)applicationDocumentsDirectory {
    // The directory the application uses to store the Core Data store file. This code uses a directory named "gml.2016.FIGHT.ESP3" in the application's documents directory.
...
- (NSManagedObjectModel *)managedObjectModel {
    ...
    NSURL *modelURL = [[NSBundle mainBundle] URLForResource:@"ESP3" withExtension:@"momd"];
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator {
    ...
    NSURL *storeURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:@"ESP3.sqlite"];
    NSError *error = nil;
